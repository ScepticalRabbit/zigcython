...
def add_vec(v0: cython.double[::1],
    ...
    # Allocate the output numpy array to return
    v_out_np = np.full((v0.shape[0],),0.0,dtype=np.float64)
    #v_out_np = np.ascontiguousarray(v_out_np)
    v_out: cython.double[::1] = v_out_np

    # Call the zig dynamic library to do the addition
